...
app.post('/api/admin/login', async (req, res) => {
  ...
  try {
    ...
    const currentPassword = (s && s.adminPassword) ? s.adminPassword : (process.env.ADMIN_PASSWORD || '1234');
...
app.put('/api/admin/settings', requireAdmin, async (req, res) => {
  try {
    const updateData = { ...req.body, key: 'global' };
    if (!updateData.adminPassword) {
      delete updateData.adminPassword; // Prevent wiping password if left blank
    }
    const s = await Setting.findOneAndUpdate({ key: 'global' }, updateData, { new: true, upsert: true });
    const data = s ? s.toObject() : {};
    delete data.adminPassword; // Don't return password
    res.json(data);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
app.get('/api/settings', async (req, res) => {
  try {
    const s = await Setting.findOne({ key: 'global' });
    const data = s ? s.toObject() : {};
    delete data.adminPassword; // Security: do not leak password
    res.json(data);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
